...
test.describe('Homepage tests', () => {
  test.beforeEach(async ({ page }) => {
    ...
    await page.waitForSelector('main', { state: 'visible', timeout: 30000 }); // Increased timeout
  });
  ...
  test('check if navigation menu is present', async ({ page }) => {
    try {
      await page.waitForSelector('nav', { state: 'visible', timeout: 30000 }); // Increased timeout
  ...
  test('check if event cards are present', async ({ page }) => {
    try {
      await page.waitForSelector('article[data-testid="event-card"]', { state: 'visible', timeout: 30000 }); // Increased timeout
