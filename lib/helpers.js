...
export async function getCategorizedEvents({
  ...
  try {
    ...
    const fetchEventsForTerm = async (term) => {
      ...
      let items = gcalResponse?.items || [];
      items = items.filter((i) => !i.summary.includes("Ad"));
      const normalizedEvents = filterByDate
      ...
      if (normalizedEvents.length) {
        ...
        const ads = Array.from(
          { length: Math.ceil(normalizedEvents.length / 4) },
          (_, i) => ({
            ...adEvent,
            id: i,
          })
        ...
        return insertAdsRandomly(normalizedEvents, ads);
      }

      return normalizedEvents;
    };

    const eventPromises = searchTerms.map(fetchEventsForTerm);
    const eventsResults = await Promise.all(eventPromises);

    searchTerms.forEach((term, index) => {
      categorizedEvents[term] = eventsResults[index];
    });
