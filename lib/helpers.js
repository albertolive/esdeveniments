...
export async function getCalendarEvents({
  ...
  try {
    ...
    normalizedEvents = shuffleAndMergeEvents(normalizedEvents);
...
function shuffleAndMergeEvents(events) {
  const currentDate = new Date();

  let shuffleEvents = events.filter(
    (event) =>
  ...
  shuffleEvents = shuffleArray(shuffleEvents);

  return [...shuffleEvents, ...otherEvents];
}
