...
export async function getCalendarEvents({
  ...
  try {
    ...
    while (items.length < maxResults && !allEventsLoaded) {
      ...
      }@group.calendar.google.com/events?timeMin=${fromDate}${untilDate}&q=${q}&singleEvents=true&orderBy=startTime&maxResults=${maxResults}&key=${
        process.env.NEXT_PUBLIC_GOOGLE_CALENDAR_ID
      }${pageToken ? `&pageToken=${pageToken}` : ""}`;

      const [gcalResponse, weatherResponse] = await Promise.all([
        fetch(gcalUrl).then((res) => res.json()),
        getWeather(normalizeRss),
      ]);

      const newItems = gcalResponse?.items?.filter(
        (i) => !i.summary.includes("Ad")
      );

      if (hideMultiDay) {
        items = [...items, ...newItems.filter((item) => !item.isMultipleDays)];
      } else {
        items = [...items, ...newItems];
      }

      pageToken = gcalResponse.nextPageToken;
      weather = weatherResponse;

      if (!pageToken || newItems.length < maxResults) {
    ...
    if (shuffleItems) {
      normalizedEvents = shuffleArray(normalizedEvents, hideMultiDay);
    }
...
function shuffleArray(array, hideMultiDay) {
  ...
  return todaysSingleDayEvents.concat(
    hideMultiDay ? [] : multiDayEvents,
    otherEvents
  );
}
