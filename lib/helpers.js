...
function insertAdsRandomly(
  ...
  if (!Array.isArray(array) || !Array.isArray(ads)) {
    console.error("Array or ads is undefined or not an array");
    return [];
  }
  ...
  ads.forEach((ad, i) => {
    ...
    if (index < newArray.length) {
      ...
      console.error("No valid positions left for inserting ads.");
    }
  });
...
export async function getCalendarEvents({
  ...
  try {
    ...
    if (!Array.isArray(gcalResponse?.items)) {
      console.error("gcalResponse.items is undefined or not an array");
      return {
        events: [],
        noEventsFound: true,
      };
    ...
    let normalizedEvents = (normalizedEvents = items
      ? filterByDate
    ...
    if (!Array.isArray(normalizedEvents)) {
      console.error("normalizedEvents is undefined or not an array");
      return {
        events: [],
        noEventsFound: true,
      };
